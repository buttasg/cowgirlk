...
Autocomplete.prototype = {
  ...
  suggest: function() {
    ...
    var totalResults=this.suggestions.length;
    totalResults=totalResults-4;
    this.suggestions.each(function(value) {
	//alert(totalResults+' '+i);
        if (value.html) {
            n++;
	...
	var price = value.price ?  '<br /><span class="ajaxsearchprice">' + value.price + '</span>' : ''
        var p = '<p>';
        if (previousIsHtml) {
            previousIsHtml = false;
            p = '<p class="ajaxsearch-small">';
        }
        var lastClass=(totalResults==i)?'class="last_item"':'';
        content.push(
            (this.selectedIndex === i ? '<div class="selected ajaxsearchtitle"' : '<div class="ajaxsearchtitle"'),
            ' title="', value.name, lastClass,
