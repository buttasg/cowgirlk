...
{$(errorDivId).html('Please wait...').removeClass('no-dispaly');$.ajax({type:settings.method,url:settings.url,dataType: "json",data:{email:email.val()},error:function(){$(errorDivId).removeClass('no-dispaly').html('An error occured. Please try later.')},success:function(data)
{$(errorDivId).html('').addClass('no-dispaly');$(settings.textId).val('');showWaterMark();applyFade();
if(data.error){$(errorDivId).html(data.message).removeClass('no-dispaly');}
if(data.success)
