...
class As_Inventory_Model_Sale extends Mage_Sales_Model_Order_Pdf_Abstract 
{
    ...
    {   
    	...
    	$select->where('s.created_at>=?',$this->from);
    	$select->where('s.created_at<=?',$this->to);
    	//echo $select; die();
    ...
    public function getTotalTexasSale()
    {
    	...
    	$read=$resouce->getConnection('core_read');
    	$saleTable=$resouce->getTableName('sales/order');
    	$addressTable=$resouce->getTableName('sales/order_address');//grand_total
    	$select=$read->select()->from(array('s'=>$saleTable),array('total_sale'=>new Zend_Db_Expr('SUM(s.subtotal)')));
