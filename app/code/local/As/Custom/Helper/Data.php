...
class As_Custom_Helper_Data extends Mage_Core_Helper_Abstract
{
	...
	protected function _getCollection()
	{
		...
		self::$_collection=Mage::getModel('catalog/category')->getCollection();
		}*/
		
		return Mage::getModel('catalog/category')->getCollection();
	}
	
	public function getHomeleftCategories()
	{
		...
		$collection->addAttributeToFilter('show_left',1);
	...
	{
		$collection=$this->_getCollection();
		$collection->addAttributeToSelect('url_key')->addAttributeToSelect('name');
		$collection->addAttributeToFilter('is_active', 1);
		$collection->addAttributeToFilter('level',2);
		$collection->addAttributeToFilter('home_page',1);
		$collection->setOrder('position','ASC');
		return $collection;
	}
	...
	{	
		...
		$collection->addAttributeToFilter('deal_of_the_day',1)
	...
	{	$brandCategoryId=Mage::getStoreConfig('brand/brand/category_id');
		$brand=Mage::getModel('catalog/category')->load($brandCategoryId);
		$prodCat=$product->getCategoryIds();
		$categories=$brand->getChildren();
		$a=explode(',',$categories);
		//echo '<pre>';
		//print_r($prodCat);
		//print_r($a);
		$b=array_intersect($a,$prodCat);
		//print_r($b);
		if(count($b))
		{   $brandId=reset($b);
		    $brandCat=Mage::getModel('catalog/category')->load($brandId);
		    return $brandCat->getName();
		}
	}
	public function ufcase($text)
	{
		return ucfirst(strtolower($text));
	}
	
	public function saveCart($html)
	{     $seesionId=session_id();
	      $filename=BP.DS."var".DS."wp".DS.$seesionId;
	     /*$quoteId=Mage::getModel('checkout/cart')->getQuote()->getId();
	     if(!empty($quoteId)){
	     	 $filename=BP.DS."var".DS."wp".DS."quote_".$quoteId;
	     	file_put_contents($filename,$html);
	     }*/
	     file_put_contents($filename,$html);
	     $_COOKIE['bridge']=$seesionId;
	}
	...
	public function htmlspecialchars($html)
	{
		return htmlspecialchars($html);
	}
	
	public function escapeCharacters($html)
	{
		return str_replace(array('"','&'),array('\'','&amp;'),$html);
	}
	
	public function getCommonProtocolUrl($url)
	{
